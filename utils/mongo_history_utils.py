...
def save_chat_message(
    ...
    document = {
        ...
        "references": references or [],  # 参考资料列表
        "ts": ts  # 时间戳，排序和时间筛选维度
    }
...
def get_all_sessions() -> List[Dict[str, Any]]:
    """
    获取所有会话列表，每个会话返回 session_id 和第一条用户消息作为标题
    按最新消息时间倒序排列
    :return: 会话列表
    """
    mongo_tool = get_history_mongo_tool()
    try:
        ...
        result = mongo_tool.chat_message.aggregate(pipeline)
        return list(result)
    except Exception as e:
...
if __name__ == "__main__":
    # # 测试代码
    # save_chat_message(
    #     "test_session_id_001", "user", "请问这个怎么用？", "请问HAK180烫金机如何使用？"
    # )
