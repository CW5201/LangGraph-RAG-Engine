...
class NodeImportMilvus(BaseNode):
    ...
    def _create_chunks_collection(self, collections_name, milvus_client, vector_dimension):
        ...
        schema.add_field(field_name="title", datatype=DataType.VARCHAR, max_length=200)  # 切片标题
        schema.add_field(field_name="parent_title", datatype=DataType.VARCHAR, max_length=200)  # 父标题
        schema.add_field(field_name="part", datatype=DataType.INT8)  # 分片编号
        schema.add_field(field_name="file_title", datatype=DataType.VARCHAR, max_length=200)  # 源文件标题
        schema.add_field(field_name="item_name", datatype=DataType.VARCHAR, max_length=200)  # 商品名称（幂等性依据）
