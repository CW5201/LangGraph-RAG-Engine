...
class NodeAnswerOutput(NodeBase):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        references = self._extract_references_from_docs(state.get("reranked_docs") or [])

        # 阶段五：把答案写入到mongodb的history中
        if state.get("answer"):
            logger.info("---写入MongoDB历史记录---")
            self._step_4_write_history(state, image_urls=image_urls, references=references)

        add_done_task(state['session_id'], self.name, state.get("is_stream"))

        # 阶段六: 流式输出结束，发送 final 事件
        logger.info(f"---发送 final 事件---图片为：{image_urls}，参考资料：{references}")
    ...
    def _step_1_check_answer(self, state) -> bool:
          ...
          if answer:
            if is_stream:
              ...
              # 存储参考资料供非流式响应使用
              refs = self._extract_references_from_docs(state.get("reranked_docs") or [])
              set_task_result(state["session_id"], "references", refs)
            return True
          else:
            return False
    ...
    def _step_3_generate_response(self, state: QueryGraphState, prompt: str) -> QueryGraphState:
        ...
        if is_stream:
            ...
            try:
                ...
                # 存储参考资料供非流式响应使用
                refs = self._extract_references_from_docs(state.get("reranked_docs") or [])
                set_task_result(session_id, "references", refs)
    ...
    def _extract_references_from_docs(self, docs):
        """
        从文档列表中提取参考资料名称（去重）
        返回格式：[{"name": "文件名", "source": "local/web", "url": "..."}]
        """
        refs = []
        seen = set()
        if not docs:
            return refs
        for doc in docs:
            name = (doc.get("title") or doc.get("file_name") or "").strip()
            ...
            if key in seen:
                continue
            seen.add(key)
            refs.append({"name": name, "source": source, "url": url})
        return refs

    def _step_4_write_history(seld, state: QueryGraphState, image_urls=None, references=None) -> QueryGraphState:
        """
